...
function Card({ item, tab }: { item: Offer | Demand; tab: Tab }) {
  ...
  const price = isOffer
    ? (item as Offer).price_per_kg
    : (item as Demand).max_price_per_kg;
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md hover:border-emerald-300 transition">
      <div className="flex items-start justify-between mb-3">
        ...
        <span
          className={`text-xs font-medium px-2 py-0.5 rounded ${
            isOffer ? "bg-blue-50 text-blue-700" : "bg-amber-50 text-amber-700"
          }`}
        >
      ...
      <div className="space-y-1.5 text-sm">
        ...
        <Row
          label={isOffer ? "Цена" : "Макс. цена"}
          value={`${price} ₽/кг`}
        />
        <Row label="Срок поставки" value={`${item.delivery_days} дн.`} />
      </div>

      <button className="mt-4 w-full py-2 rounded-lg bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 transition">
        {isOffer ? "Найти покупателей" : "Найти поставщиков"}
      </button>
    </div>
  );
}
